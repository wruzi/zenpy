...
let activeSortMode = 'price_asc';
const bannerAccentMap = {};

const premiumBannerPrefixes = ['banner_video_'];

function isGifAssetPath(assetPath) {
    return /\.gif(?:\?.*)?$/i.test(String(assetPath || ''));
}

function isPremiumMediaBanner(item) {
    if (!item || item.category !== 'profile_banner') return false;
    return premiumBannerPrefixes.some(prefix => String(item.id || '').startsWith(prefix))
        || Boolean(item.isAutoGenerated)
        || isGifAssetPath(item.assetPath);
}
...
const categoryOrderNamesToCard = [
    'name_style',
    'avatar_frame',
    'effect',
    'name_accessory',
...
function setPromoResult(message, type = 'info') {
    const resultEl = document.getElementById('promoRedeemResult');
    if (!resultEl) return;
    resultEl.textContent = message || '';
    resultEl.style.color = type === 'success'
        ? 'var(--success)'
        : type === 'error'
            ? 'var(--error)'
            : 'var(--text-muted)';
...
async function redeemPromoCode() {
    ...
    if (!code) {
        setPromoResult('Enter a promo code first.', 'error');
        return;
    }

    if (code.length > 10) {
        setPromoResult('Promo code max length is 10.', 'error');
        return;
    }

    setPromoResult('Redeeming...', 'info');

    const data = await apiCall('/api/promo-codes/redeem', {
        method: 'POST',
        body: JSON.stringify({ code })
    });

    if (!data?.success) {
        setPromoResult(data?.message || 'Promo redeem failed.', 'error');
        showToast(data?.message || 'Promo redeem failed.', 'error');
        return;
    }

    const zenReward = Number(data?.rewards?.zen || 0);
    const itemNames = Array.isArray(data?.rewards?.itemNames) ? data.rewards.itemNames : [];
    const itemRewardMsg = itemNames.length > 0 ? ` | Items: ${itemNames.join(', ')}` : '';

    userData.zen = data.zen;
    userData.inventory = data.inventory;
    userData.cssMap = buildCssMapFromEquipped(data.inventory?.equipped || {});
    ...
    const successMsg = `Redeemed! Zen +${zenReward}${itemRewardMsg}`;
    setPromoResult(successMsg, 'success');
    showToast('Promo code redeemed successfully!', 'success');
}

function getSortBucket(item) {
    if (!item) return 'profile_card';
    if (isPremiumMediaBanner(item)) return 'premium';
    if (['chat_extra', 'chat_color', 'chat_background'].includes(item.category)) return 'chat';
    return item.category;
}

function getCategoryRank(item, mode) {
    const bucket = getSortBucket(item);
    const baseRank = categoryOrderNamesToCard.indexOf(bucket);
    const safeRank = baseRank >= 0 ? baseRank : categoryOrderNamesToCard.length;
    if (mode === 'category_card_to_name') {
        return categoryOrderNamesToCard.length - safeRank;
    }
    return safeRank;
}
...
function sortShopItems(items, mode) {
    ...
    sorted.sort((a, b) => {
        ...
        if (mode === 'category_name_to_card' || mode === 'category_card_to_name') {
            const categoryDelta = getCategoryRank(a, mode) - getCategoryRank(b, mode);
...
function renderShop(category = 'all') {
    ...
    const sorted = sortShopItems(filtered, activeSortMode);
...
function toggleSortMenu(event) {
    ...
    const menu = document.getElementById('shopSortMenu');
    if (!menu) return;
...
function updateSortMenuUI() {
    document.querySelectorAll('.sort-option').forEach(option => {
        option.classList.toggle('active', option.dataset.sort === activeSortMode);
    });
}

function handleShopSortMenuOutsideClick(event) {
    const menu = document.getElementById('shopSortMenu');
    if (!menu) return;
    if (!menu.contains(event.target)) closeSortMenu();
}
...
document.addEventListener('DOMContentLoaded', () => {
    document.addEventListener('click', handleShopSortMenuOutsideClick);
    const promoInput = document.getElementById('promoCodeInput');
    if (promoInput) {
        promoInput.addEventListener('input', () => {
            promoInput.value = promoInput.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 10);
        });
    }
    loadShop();
});
